...
function registerProperty(prop)
    ...
    local rTable = {}
    local loaded = false
    MySQL.Async.fetchAll("SELECT * FROM `properties` WHERE `property_id` = @prop", {['@prop'] = prop}, function(propSQL)
        if propSQL[1] ~= nil then
            self.pid                = propSQL[1].property_id
            self.name               = propSQL[1].name
            self.storageInvLimit    = propSQL[1].storageLimit
            self.basePrice          = propSQL[1].purchaseCost
            if propSQL[1].radials ~= nil then
                self.radiusLimits   = json.decode(propSQL[1].radials)
            ...
            if propSQL[1].metainformation ~= nil then
                self.metaInformation                        = json.decode(propSQL[1].metainformation)
                ...
                self.metaInformation['locations']           = { ['backInside'] = self.DefaultCoords, ['backEntrance'] = self.DefaultCoords, ['clothing'] = self.DefaultCoords, ['money'] = self.DefaultCoords, ['weapon'] = self.DefaultCoords, ['inventory'] = self.DefaultCoords, ['garage'] = self.DefaultCoords, ['location'] = json.decode(propSQL[1].location), ['management'] = json.decode(propSQL[1].manager), ['inside'] = json.decode(propSQL[1].inside), ['charSpawn'] = json.decode(propSQL[1].charSpawn), ['property'] = json.decode(propSQL[1].location) }
                ...
                self.metaInformation['costs']               = { ['purchase'] = propSQL[1].purchaseCost, ['rental'] = propSQL[1].purchaseCost / 100 / 2 }
            ...
            if propSQL[1].furniture ~= nil then
                self.furniture = json.decode(propSQL[1].furniture)
            else
                self.furniture = {}
    ...
    rTable.getStorageLimit = function()
        return exports['pw_inventory']:invLimit(tonumber(self.storageInvLimit))
    end
    ...
    rTable.addMultiplePendingFurniture = function(data, method, cid)
        ...
        for i = 1, #data do
            local sendMeta = {  ['prop'] = data[i].prop,
