...
local showMarker, showingInfo, onVeh, waitingFuel, waitingPump, taxi, gotCan = false, false, false, false, false, { ['veh'] = 0, ['ped'] = 0 }, false
local garages, privateGarages, units, prevSlots, prevPrivSlots, prevUnitSlots, prevUnitOwner, blips, unitBlips, insuranceBlips, spawnedVehicles = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
...
RegisterNetEvent('pw_garage:client:insureTest')
AddEventHandler('pw_garage:client:insureTest', function()
    if spawnedVehicles ~= nil and spawnedVehicles[1] ~= nil then
        for k,v in pairs(spawnedVehicles) do
            if v.insurance.plan > 0 then
                HandleInsurance(v)
...
AddEventHandler('pw_interact:closeMenu', function()
    if inParkedVehs then
        OpenGarage(inParkedVehs.type, inParkedVehs.garage)
    end
end)
...
AddEventHandler('pw_garage:client:spawnVehicle', function(type, props, id, insurance, damage)
    local coords
    if type == 'Public' or type == 'Impound' then
    ...
    PW.Game.SpawnOwnedVehicle(props.model, coords, coords.h, function(vehicle)
        ...
        if type == 'Private' or type == 'Unit' then SetPedIntoVehicle(PlayerPedId(), vehicle, -1); end
...
function OpenGarage(type, id)
    inParkedVehs = false
    local ped = PlayerPedId()
    local checkVeh
    if type == 'Public' or type == 'Private' or type == 'Unit' then
    ...
    if checkVeh then
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            if CanPark(type, id) then
                ...
                end, PW.Vehicles.GetVinNumber(props.plate))
            else
        ...
        PW.TriggerServerCallback('pw_garage:server:getParkedVehicles', function(vehs)
            if vehs then
                for k,v in pairs(vehs) do
                    ...
                    table.insert(menu, { ['label'] = GetVehNameByModel(meta.model), ['action'] = 'pw_garage:client:openVehicleStats', ['value'] = {type = type, garage = id, props = props, plate = v.plate, name = GetVehNameByModel(meta.model)}, ['triggertype'] = 'client', ['color'] = 'primary' })
                end
            else
            ...
            TriggerEvent('pw_interact:generateMenu', menu, (type == 'Public' and 'Garage | '..garages[id].name or type == 'Private' and 'Private Garage | '..privateGarages[id].name or type == 'Impound' and 'Impound Lot | '..garages[id].name or type == 'Unit' and 'Garage Unit #'.. id .. ' | ' .. units[id].name))
        end, type, id)
    end
end
...
function ShowImpoundCostForm(data)
    local form = {
        ...
        { ['type'] = "hidden", ['name'] = "veh", ['data'] = {type = data.type, plate = PW.Vehicles.GetVinNumber(data.plate)}}
    }
...
AddEventHandler('pw_garage:client:checkClearPathForSpawn', function(data)
    local closestVeh
    if data.type == 'Public' or data.type == 'Impound' then
    ...
    if closestVeh == 0 or closestVeh == nil then
        inParkedVehs = false
        if data.type == 'Impound' then
            ShowImpoundCostForm(data)
        else
            TriggerServerEvent('pw_garage:server:takeVehicle', data.type, PW.Vehicles.GetVinNumber(data.plate))
        end
    else
        exports.pw_notify:SendAlert('error', 'There\'s a vehicle blocking the exit', 4000)
    end
end)
...
AddEventHandler('pw_garage:client:openVehicleStats', function(data)
    ...
    if data.type == 'Impound' then
        ...
        table.insert(infoSub, { ['label'] = "Take Out", ['action'] = 'pw_garage:client:checkClearPathForSpawn', ['value'] = {plate = data.props.plate, garage = data.garage, type = data.type}, ['triggertype'] = 'client', ['color'] = 'info'})
        table.insert(infoSub, { ['label'] = "Plate: "..data.props.plate, ['action'] = '', ['triggertype'] = 'client', ['color'] = 'primary'})
    ...
    TriggerEvent('pw_interact:generateMenu', infoSub, data.name)
end)
...
function ShowPrivateGarage(id)    
    Citizen.CreateThread(function()
        while showMarker == 'private'..id do
            ...
            local checkVeh = IsPedInAnyVehicle(PlayerPedId(), false)
...
AddEventHandler('pw_garage:client:signThisVeh', function(data)
    ...
    local form = {
        ...
        { ['type'] = "writting", ['align'] = 'center', ['value'] = '<b>FUEL DELIVERY SERVICE</b><br>The closest Service Provider will arrange a fuel delivery service will supply you with an emergency jerrycan.' },
    ...
    PW.TriggerServerCallback('pw_garage:server:getOwnedVehs', function(vehs)
        ...
        if myVehs then
            for k,v in pairs(myVehs) do
                table.insert(menu, { ['label'] = v.plate .. ' (' .. GetVehNameByModel(v.model) .. ')', ['action'] = (not v.cooldown and 'pw_garage:client:signThisVeh' or ''), ['value'] = { ['vin'] = v.vin, ['plan'] = plan, ['model'] = v.model }, ['triggertype'] = 'client', ['color'] = (not v.cooldown and 'primary' or 'danger') })
                if v.cooldown then
                    local sub = {}
...
AddEventHandler('pw_garage:client:endInsurance', function(vin)
    local form = {
        ...
        { ['type'] = 'hidden', ['name'] = 'vin', ['value'] = vin  }
    }
...
AddEventHandler('pw_garage:client:retrieveInsuredVeh', function(vin)
    local menu = {}
    ...
    for k,v in pairs(garages) do
        if v.type == 'Public' then
            ...
            if not full then
                local sub = {}
                table.insert(sub, { ['label'] = 'Choose as destination', ['action'] = 'pw_garage:client:retrieveToGarage', ['value'] = { vin = vin, garage = k }, ['triggertype'] = 'client' })
                menu[#menu]['subMenu'] = sub
            end
        end
    end
...
AddEventHandler('pw_garage:client:loadInsurance', function(vin)
    local sub = {}
    local menu = {}
    PW.TriggerServerCallback('pw_garage:server:loadInsurance', function(info)
        if info.plan > 0 then
            if info.insured then
                table.insert(sub, { ['label'] = 'Retrieve vehicle', ['action'] = 'pw_garage:client:retrieveInsuredVeh', ['value'] = vin, ['triggertype'] = 'client' })
            ...
            if not info.insured then
                table.insert(menu, { ['label'] = 'Cancel Insurance', ['action'] = 'pw_garage:client:endInsurance', ['value'] = vin, ['triggertype'] = 'client', ['color'] = 'danger' })
            end
        else
...
AddEventHandler('pw_garage:client:openVehsInsurance', function(vehs)
    ...
    if vehs then
        for k,v in pairs(vehs) do
            table.insert(menu, { ['label'] = v.plate .. ' (' .. GetVehNameByModel(v.model) .. ')', ['action'] = 'pw_garage:client:loadInsurance', ['value'] = v.vin, ['triggertype'] = 'client', ['color'] = 'primary' })
        end
    else
        table.insert(menu, { ['label'] = "No owned vehicles", ['color'] = 'danger disabled' })
    end
    ...
    Citizen.CreateThread(function()
        while waitingPump do
            ...
            if dist < 50.0 then
                ...
                local closestPedNode, closestNodePos = GetClosestVehicleNode(Px, Py, Pz, 1, 3.0, 0)
...
Citizen.CreateThread(function()
    while true do
        if playerLoaded and garages ~= nil and #garages > 0 then
            ...
            local pedCoords = GetEntityCoords(ped)
            for i = 1, #garages do
                if garages[i].type == 'Public' or garages[i].type == 'Impound'  then
