...
AddEventHandler('pw_chopshop:server:signUp', function(data)
    ...
    if data.email.value ~= nil and string.len(data.email.value) > 16 and string.sub(data.email.value, -17) == '@pixelworldrp.com' and not HasEmail(data.email.value) then
        local charEmail = _char:Character().getEmail()
        if data.email.value == charEmail then
            table.insert(Signed, { ['cid'] = _char:Character().getCID(), ['email'] = charEmail, ['cooldown'] = false, ['streak'] = 0 })
...
AddEventHandler('pw_chopshop:server:sendNpcLocation', function(coords)
    ...
    local message = "If you want to show what you are worth to us, meet our contact at the attached location, alone.<br><b>NO COPS</b>."
    ...
    if not activeService then
        if #Signed > 0 then
            ...
            repeat
                ...
                online = exports.pw_base:checkOnline(Signed[chosen].cid)
                if not online then
                    Signed[chosen] = nil
                end
            until online ~= false
