...
PW.RegisterServerCallback('pw_pedfeatures:server:getFeatures', function(source, cb)
    local _char = exports['pw_base']:Source(source)
    MySQL.Async.fetchScalar('SELECT `features` FROM `characters` WHERE `cid` = @cid', { ['@cid'] = _char:Character().getCID() }, function(settings)
        if settings ~= nil then
            cb(json.decode(settings))
        else
            settings = { ['mood'] = 'default', ['walk'] = 'default' }
            MySQL.Async.execute('UPDATE `characters` SET `features` = @features WHERE `cid` = @cid', { ['@features'] = json.encode(settings), ['@cid'] = _char:Character().getCID() }, function() end)
            cb(settings)
        end
    end)
end)
...
AddEventHandler('pw_pedfeatures:server:saveFeatures', function(features)
    ...
    MySQL.Async.execute('UPDATE `characters` SET `features` = @features WHERE `cid` = @cid', { ['@features'] = json.encode(features), ['@cid'] = _char:Character().getCID() }, function()
