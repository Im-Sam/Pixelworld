...
AddEventHandler('pw_bankrobbery:server:readCard', function(bank, hours, universal, data)
    ...
    _char:Inventories():AddItem():Player().Slot('vaultcard', 1, data.slot, {['public'] = { ['decoded'] = true }, ['private'] = json.decode(data.metaprivate)})
end)
...
AddEventHandler('pw_bankrobbery:server:awardVaultGoods', function(bank)
    ...
    if Banks[bank].bankType == 'Small' then
        ...
        if awardGoods > 0 then
            _char:Inventories():AddItem():Player().Single('valuegood', awardGoods)
    ...
    elseif Banks[bank].bankType == 'Big' then
        ...
        _char:Inventories():AddItem():Player().Single('moneybag', 1, {['public'] = { ['amount'] = bagMoney }})
...
AddEventHandler('pw_bankrobbery:server:counterRewards', function(bank)
    ...
    if cardChance <= Config.BankCardChance then
        ...
        if universalCardChance <= Config.UniversalCardChance then
            _char:Inventories():AddItem():Player().Single('vaultcard', 1, {['private'] = { ['hours'] = {0, 0}, ['bank'] = 0, ['bankName'] = "Universal", ['reader'] = cardReaderSpot }})
        else
            ...
            _char:Inventories():AddItem():Player().Single('vaultcard', 1, {['private'] = { ['hours'] = {firstHour, secondHour}, ['bank'] = bank, ['bankName'] = Banks[bank].name, ['reader'] = cardReaderSpot }})
        end
