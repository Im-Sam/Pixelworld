...
AddEventHandler('pw:characterLoaded', function(source)
    ...
    MySQL.Async.fetchAll('SELECT * FROM `character_pets` WHERE `character_id` = @cid', {['@cid'] = _char:Character().getCID()}, function(result)
        if result then
            local tempTable = {}
...
AddEventHandler('pw_pets:server:returnEmptyBowl', function()
    local _src = source
    local _char = exports['pw_base']:Source(_src)
    _char:Inventories():AddItem():Player().Single("dogbowl", 1, {['public'] = {}, ['private'] = {} })
end)
...
AddEventHandler('pw_pets:server:useItem', function(id, data)
    ...
    if takeItem[1] ~= nil then
        _char:Inventories():Remove().byName(takeItem[1].name, takeItem[1].qty)
    end
    ...
    if returnItem[1] ~= nil then
        _char:Inventories():AddItem():Player().Single(returnItem[1].name, returnItem[1].qty, {['public'] = {}, ['private'] = {} })
    end
end)
...
AddEventHandler('pw_pets:server:buyPet', function(data)
    ...
    if _char:Cash().getCash() >= petCost then
        _char:Cash().Remove(petCost)
        MySQL.Async.insert('INSERT INTO `character_pets` (`character_id`, `pet_hash`, `pet_name`, `pet_color`) VALUES (@cid, @petHash, @petName, @petColor)', {['@cid'] = _char:Character().getCID(), ['@petHash'] = Config.Pets[breed].hash, ['@petName'] = pet, ['@petColor'] = (data.color or 0) }, function(rid)
            TriggerClientEvent('pw:notification:SendAlert', _src, {type = 'success', text = 'You just bought yourself a '..breed })
...
PW.RegisterServerCallback('pw_pets:server:getOwnedProperties', function(source, cb)
    local _src = source
    local _char = exports.pw_base:Source(_src)
    cb(_char:Character().myProperties())
end)
