...
MySQL.ready(function ()
    ...
    for k, v in pairs(Config.Locations) do 
        exports.pw_banking:createBuisnessAccount('mechanic', k)
    end
...
AddEventHandler('pw_mechanic:server:getVeh', function(data)
    local _src = source
    MySQL.Async.execute("UPDATE `owned_vehicles` SET `stored_garage` = 0, `stored_garagetype` = 'None', `stored_garageid` = 0 WHERE `vin` = @vin", {['@vin'] = data.vin }, function() 
...
AddEventHandler('pw_mechanic:server:storeVehicle', function(vin, garage)
    MySQL.Async.execute("UPDATE `owned_vehicles` SET `stored_garage` = 1, `stored_garagetype` = 'Business', `stored_garageid` = @garage WHERE `vin` = @vin", {['@vin'] = vin, ['@garage'] = garage }, function() end)
end)

RegisterServerEvent('pw_mechanic:server:updateProps')
AddEventHandler('pw_mechanic:server:updateProps', function(vin, props, vNet)
    local vehicle = exports.pw_vehicleshop:getVehicleByVin(vin)
    if vehicle then
...
AddEventHandler('pw_mechanic:server:checkout', function(parts, garage)
    ...
    for k,v in pairs(parts) do
        if v.repair == nil then
            local partsCount = exports.pw_inventory:checkItemCount(25, garage, v.item, v.qty)
...
AddEventHandler('pw_mechanic:server:pinEntered', function(data)
    ...
    for i = 1, #parts do
        if parts[i].repair == nil then
            local partsCount = exports.pw_inventory:checkItemCount(25, garage, parts[i].item, parts[i].qty)
    ...
    if allGood then
        local bank = exports.pw_banking:buisness('mechanic', garage)
        local veh = exports.pw_vehicleshop:getVehicleByPlate(plate)
        for i = 1, #parts do
            local partValue 
            if parts[i].repair == nil then
                partValue = parts[i].hours + ((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and parts[i].partsCost or 0)
                exports['pw_inventory']:removeItemFromInventory(25, garage, parts[i].item, parts[i].qty)
                bank.addBalance(partValue, 'Installation of '..parts[i].itemLabel..' (Hour Rate: $'..parts[i].hours..((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and ' + Parts Cost: $'..parts[i].partsCost or "")..') on vehicle with plate '..plate.." (VIN: "..vin..")")
            else
                partValue = parts[i].partsCost
                bank.addBalance(partValue, 'Service: '..parts[i].label.. ((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and 'Service Cost: $'..parts[i].partsCost or "")..') on vehicle with plate '..plate.." (VIN: "..vin..")")
            end
        end
...
AddEventHandler('pw_mechanic:server:payStuff', function(data)
    ...
    if data.type == 'cash' then
        if _char.Cash().getCash() >= totalAmount then
            local allGood = true
            for i = 1, #parts do
                if parts[i].repair == nil then
                    local partsCount = exports.pw_inventory:checkItemCount(25, tonumber(data.stuff.garage.value), parts[i].item, parts[i].qty)
                    if partsCount < parts[i].qty then
                        allGood = false
                        break
                    end
            ...
            if allGood then
                _char:Cash().Remove(totalAmount)
                local bank = exports.pw_banking:buisness('mechanic', tonumber(data.stuff.garage.value))
                local veh = exports.pw_vehicleshop:getVehicleByPlate(data.stuff.veh.data.plate)
                for i = 1, #parts do
                    local partValue 
                    if parts[i].repair == nil then
                        partValue = parts[i].hours + ((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and parts[i].partsCost or 0)
                        exports['pw_inventory']:removeItemFromInventory(25, tonumber(data.stuff.garage.value), parts[i].item, parts[i].qty)
                        bank.addBalance(partValue, 'Installation of '..parts[i].itemLabel..' (Hour Rate: $'..parts[i].hours..((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and ' + Parts Cost: $'..parts[i].partsCost or "")..') on vehicle with plate '..data.stuff.veh.data.plate.." (VIN: "..veh..")")
                    else
                        partValue = parts[i].partsCost
                        bank.addBalance(partValue, 'Service: '..parts[i].label.. ((parts[i].partsCost ~= nil and parts[i].partsCost > 0) and 'Service Cost: $'..parts[i].partsCost or "")..') on vehicle with plate '..data.stuff.veh.data.plate.." (VIN: "..veh..")")
                    end
                end
                TriggerClientEvent('pw:notification:SendAlert', _src, {type = 'success', text = 'Payment successful'})
...
function CreatePendingInstall(vin, plate, mech, garage, parts)
    ...
    local meta = { ['mech'] = exports.pw_base:Source(mech):Character().getName(), ['garage'] = garage, ['date'] = os.date('%d/%m/%Y %H:%M') }
    local sendMeta = json.encode(meta)
    proceed = false
    MySQL.Async.insert("INSERT INTO `pending_mechanic` (`vin`, `plate`, `install`, `meta`) VALUES (@vin, @plate, @install, @meta)", { ['@vin'] = vin, ['@plate'] = plate, ['@install'] = sendInstall, ['@meta'] = sendMeta }, function(inserted)
...
AddEventHandler('pw_mechanic:server:contractSigned', function(res)
    ...
    TriggerClientEvent('pw:notification:SendAlert', tonumber(res.bossSrc.value), {type = 'inform', text = _char.getName() .. " signed the contract and is now one of your employees"})
end)
...
PW.RegisterServerCallback('pw_mechanic:server:getPending', function(source, cb, vin, garage)
    MySQL.Async.fetchAll("SELECT * FROM `pending_mechanic` WHERE `vin` = @vin", { ['@vin'] = vin }, function(data)
        if data then
            local sent = false
            for k,v in pairs(data) do
                local dCode = json.decode(v.meta)
...
PW.RegisterServerCallback('pw_mechanic:server:getStaff', function(source, cb, garage)
    local staffList = exports.pw_base:getStaff('mechanic', garage)
    cb(staffList)
end)
        ...
        if vehicles ~= nil and countTableSize(vehicles) > 0 then
            ...
            for k,v in pairs(vehicles) do
                ...
                if meta.owner == 'mechanic' then
                    table.insert(sendTable, { ['props'] = json.decode(v.vehicle_information), ['vin'] = v.vin })
                end
            end
