    ...
    PW.TriggerServerCallback('pw_mechanic:server:getJobGarage', function(vehs)
        if vehs then
            ...
            for k,v in pairs(vehs) do
                table.insert(menu, { ['label'] = GetVehByHash(v.props.model), ['action'] = 'pw_mechanic:server:getVeh', ['value'] = {props = v.props, vin = v.vin, garage = garage}, ['triggertype'] = 'server', ['color'] = 'primary' })
            end
...
AddEventHandler('pw_mechanic:client:installPart', function(data)
    ...
    elseif cat == 'neons' then
        ...
        description = (option == 'modXenon' and (level and 'Xenon Install' or 'Xenon Removal')) or (option == 'modTurbo' and (level and 'Turbo Install' or 'Turbo Removal')) or ((level+1) > 0 and "Type: "..((option == 'pearlescentColor' or option == 'wheelColor') and level..' ('..exports.pw_base:getVehicleColor(level)..')' or (cat == 'performance' and (level+1) or level)) or "Stock")
...
AddEventHandler('pw_mechanic:client:previewSmokeColor', function(data)    
    ...
    if not smoking then
        smoking = true
        FreezeEntityPosition(curVeh.obj, false)
        TaskVehicleTempAction(PlayerPedId(), curVeh.obj, 30, 2000)
        Wait(2000)
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if GetPedInVehicleSeat(veh, -1) == ped then
                if curVeh == nil or curVeh['obj'] == nil then
                    ...
                    local vin = PW.Vehicles.GetVinNumber(curVeh.props.plate)
...
function StoreVehicle(k)
    ...
    if GetPedInVehicleSeat(pedVeh, -1) == ped then
        ...
        local vin = PW.Vehicles.GetVinNumber(props.plate)
        if vin then
            PW.TriggerServerCallback('pw_mechanic:server:getVehicleJob', function(result)
                if result == playerData.job.job and k == tonumber(playerData.job.workplace) then
...
function DrawNotif(k, id)
    ...
    elseif k == 'storage' then
        title = "Storage"
        message = "<span style='font-size: 15px'>Press <span style='color:#187200;'>[F2]</span> to access the storage</span>"
        icon = "fad fa-box-open"
        TriggerEvent('pw_inventory:client:secondarySetup', "mechanic", { type = 25, owner = id, name = "Mechanics Storage" })
    end
    if ((k == 'customize' or k == 'garage') and IsPedInAnyVehicle(PlayerPedId(), true)) or (k ~= 'customize' and k ~= 'garage') then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(50)
        if playerData and playerLoaded and playerData.job.job == 'mechanic' then
            local ped = PlayerPedId()
                    ...
                    for j,b in pairs(v) do
                        ...
                        if (j == 'bossactions' and playerData.job.grade == 'boss' and playerData.job.duty) or (j ~= 'bossactions' and ((b.dutyNeeded and playerData.job.duty) or not b.dutyNeeded)) then
                            if dist < Config.DrawDistance then
                                if not showing then
                                    ...
                                    if not previewing then
                                        DrawNotif(j, k)
                                        if ((j == 'customize' or j == 'garage') and IsPedInAnyVehicle(PlayerPedId(), true)) or (j ~= 'customize' and j ~= 'garage') then
                            ...
                            elseif showing == k..j then
                                showing = false
                                TriggerEvent('pw_drawtext:hideNotification')
                                if j == 'storage' then TriggerEvent('pw_inventory:client:removeSecondary', "mechanic"); end
                            end
                        end
                    end
                end
...
AddEventHandler('pw_mechanic:client:enterVehCheck', function(veh)
    if playerData and playerData.job.job == 'mechanic' and playerData.job.duty then
        for k,v in pairs(Config.Locations) do
...
function GetVehByHash(hash)
    local hashVehicule = hash
