...
function MFD:GetLoginData(source)
  local _src = source
  local _char = exports.pw_base:Source(_src)
  local data = MySQL.Sync.fetchAll('SELECT * FROM dopeplants WHERE owner=@owner',{['@owner'] = _char:Character().getCID()})
...
AddEventHandler('pw_base:itemUsed', function(_src,data)
  ...
  if data.item == "hgfcokeseed" or data.item == "lgfcokeseed" or data.item == "hgmcokeseed" or data.item == "lgmcokeseed" then
    TriggerClientEvent('pw_coke:Use'..data.item, _src, data)
  elseif data.item == "hgfseed" or data.item == "lgfseed" or data.item == "hgmseed" or data.item == "lgmseed" or data.item == "dopebag" or data.item == "rollingpapers" then
    TriggerClientEvent('pw_weed:Use'..data.item, _src, data)
  elseif data.item == "wateringcan" or data.item == "purifiedwater" or data.item == "lgfert" or data.item == "hgfert" then
    TriggerClientEvent('pw_coke:HandleThisShit', _src, data)
  end
end)

PW.RegisterServerCallback('pw_weed:GetLoginData', function(source,cb) cb(MFD:GetLoginData(source)); end)
PW.RegisterServerCallback('pw_weed:GetStartData', function(source,cb) while not MFD.dS do Citizen.Wait(0); end; cb(MFD.cS); end)
