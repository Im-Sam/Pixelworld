...
PW.RegisterServerCallback('pw_prison:server:checkCustodyState', function(source, cb)
    ...
    local _prisonCheck = _char:Character().getPrisonState()
...
AddEventHandler('pw_prison:server:registerPrison', function(time)
    local _src = source
    exports['pw_base']:Source(_src):Character().updatePrisonState(true, time)
end)
...
AddEventHandler('pw_prison:server:playerBrokeOut', function()
    local _src = source
    if inCustody[_src] then
        exports['pw_base']:Source(_src):Character().updatePrisonState(false, 0)
        inCustody[_src] = nil
    end
end)
    ...
    if _src ~= nil and _src > 0 then
        if inCustody[_src] then
            if inCustody[_src].timeLeft - time == 0 then
                ...
                exports['pw_base']:Source(_src):Character().updatePrisonState(true, inCustody[_src].timeLeft)
            end
        end
    end
end
...
function manageInmates()
    for k, v in pairs(inCustody) do
        ...
        if v.countUp >= 60 then
            ...
            exports['pw_base']:Source(v.source):Character().updatePrisonState(true, v.timeLeft)
        end

        if v.timeLeft <= 0 then
            exports['pw_base']:Source(v.source):Character().updatePrisonState(false, 0)
