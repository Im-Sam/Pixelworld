...
PW.TriggerServerCallback = function(name, cb, ...)
	...
	TriggerServerEvent('pw_base:server:triggerServerCallback', name, PW.CurrentRequestId, ...)
...
RegisterNetEvent('pw_base:client:serverCallback')
AddEventHandler('pw_base:client:serverCallback', function(requestId, ...)
...
PW.Game.CheckInventory = function(item)
	...
	PW.TriggerServerCallback('pw_base:server:checkItemCount', function(cou)
		count = cou
		processed = true
	end, item)
...
function skyCam()
    ...
    PointCamAtCoord(cam2, 195.55, -933.36, 29.90 + 300)
    SetCamActive(cam2, true)
    RenderScriptCams(true, true, 500, true, true)
end


RegisterNetEvent('pw_base:changeLivery')
AddEventHandler('pw_base:changeLivery', function(livery)
...
function PW.Vehicles.GetVinNumber(plate)
	local vin = nil
	PW.TriggerServerCallback('pw_base:getVinNumber', function(vvin)
		if vvin ~= nil then
			vin = vvin
		else
			vin = false
...
RegisterNetEvent('pw:spawnObject')
AddEventHandler('pw:spawnObject', function(model)
	local playerCoords = GetEntityCoords(GetPlayerPed(-1))
	PW.Game.SpawnObject(model, {
		x = playerCoords.x,
		y = playerCoords.y+1.0,
		z = playerCoords.z
	}, function(obj)
...
PW.Base.GetGradeSalery = function(job, grade)
	local processed = false
	local salery
	PW.TriggerServerCallback('pw_base:functions:getGradeSalery', function(sal)
		salery = sal
		processed = true
	end, job, grade)
	repeat Wait(0) until processed == true
...
RegisterNetEvent('pw_base:spawnVehicle')
    AddEventHandler('pw_base:spawnVehicle', function(vehicle)
    local vehicleName = vehicle or 'adder'
    
        if vehicleName == nil then 
            TriggerEvent('pw:notification:SendAlert', {type = 'inform', text = 'A Vehicle has not been specifed.'})
            return 
        end
        ...
        if not IsModelInCdimage(vehicleName) or not IsModelAVehicle(vehicleName) then
            TriggerEvent('pw:notification:SendAlert', {type = 'error', text = 'Error spawning the vehicle model you requested'})
            return
        end
		...
		local vehId = PW.Game.GetVehicleProperties(vehicle)

		PW.TriggerServerCallback('pw_vehicleshop:server:registerPotentialVin', function(vin)
			TriggerServerEvent('pw_keys:issueKey', "Vehicle", vin, false, false)
			SetPedIntoVehicle(playerPed, vehicle, -1)
			...
			-- release the model
			SetModelAsNoLongerNeeded(vehicleName)
			TriggerEvent('pw:notification:SendAlert', {type = 'inform', text = 'Vehicle spawned successfully.'})
		end, vehId, vehicle)
	end)
	
	PW.Game.DeleteVehicle = function(vehicle)
		SetEntityAsMissionEntity(vehicle, false, true)
		DeleteVehicle(vehicle)
	end
    
    RegisterNetEvent('pw_base:impoundVehicle')
    AddEventHandler('pw_base:impoundVehicle', function()
        local myPed = PlayerPedId()
        local myCoords = GetEntityCoords(myPed)
        local vehicle = GetClosestVehicle(myCoords.x, myCoords.y, myCoords.z, 5.0, 0)
		if vehicle > 0 then
			NetworkRequestControlOfEntity(vehicle)
			SetEntityAsMissionEntity(vehicle, true, true);
			DeleteEntity(vehicle)
			TriggerEvent('pw:notification:SendAlert', {type = 'inform', text = 'Vehicle deleted successfully.'})
        else 
			TriggerEvent('pw:notification:SendAlert', {type = 'error', text = 'Error deleting the Vehicle.'})
        end
	end)
