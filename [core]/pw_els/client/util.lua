...
RegisterNetEvent("pw_els:updateElsVehicles")
AddEventHandler("pw_els:updateElsVehicles", function(vehicles, patterns)
...
RegisterNetEvent("pw_els:changeLightStage_c")
AddEventHandler("pw_els:changeLightStage_c", function(sender, stage, advisor, prim, sec)
    Citizen.CreateThread(function()
...
RegisterNetEvent("pw_els:changePartState_c")
AddEventHandler("pw_els:changePartState_c", function(sender, part, newstate)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:changeAdvisorPattern_c")
AddEventHandler("pw_els:changeAdvisorPattern_c", function(sender, pat)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:changeSecondaryPattern_c")
AddEventHandler("pw_els:changeSecondaryPattern_c", function(sender, pat)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:changePrimaryPattern_c")
AddEventHandler("pw_els:changePrimaryPattern_c", function(sender, pat)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:setSirenState_c")
AddEventHandler("pw_els:setSirenState_c", function(sender, newstate)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:setHornState_c")
AddEventHandler("pw_els:setHornState_c", function(sender, newstate)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:setSceneLightState_c")
AddEventHandler("pw_els:setSceneLightState_c", function(sender)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:setCruiseLights_c")
AddEventHandler("pw_els:setCruiseLights_c", function(sender)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
RegisterNetEvent("pw_els:setTakedownState_c")
AddEventHandler("pw_els:setTakedownState_c", function(sender)
    local player_s = GetPlayerFromServerId(sender)
    local ped_s = GetPlayerPed(player_s)
    if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
        if IsPedInAnyVehicle(ped_s, false) then
...
function changeLightStage(state, advisor, PatternPrim, PatternSec)
    TriggerServerEvent("pw_els:changeLightStage_s", state, advisor, PatternPrim, PatternSec)
end

function changeAdvisorPattern(pat)
    TriggerServerEvent("pw_els:changeAdvisorPattern_s", pat)
end

function changePrimaryPattern(pat)
    TriggerServerEvent("pw_els:changePrimaryPattern_s", pat)
end

function changeSecondaryPattern(pat)
    TriggerServerEvent("pw_els:changeSecondaryPattern_s", pat)
end
...
function setSirenStateButton(state)
    ...
    if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] ~= state then
        TriggerServerEvent("pw_els:setSirenState_s", state)
...
function upOneStage()
    ...
    if GetVehicleClass(GetVehiclePedIsUsing(GetPlayerPed(-1))) == 18 then
        ...
        if(newStage == 0) then
            SetVehicleSiren(GetVehiclePedIsUsing(GetPlayerPed(-1)), false)
            TriggerServerEvent("pw_els:setSirenState_s", 0)
            TriggerServerEvent("pw_els:setDualSirenState_s", 0)
            TriggerServerEvent("pw_els:setDualSiren_s", false)
        end
    end
end
        ...
        if current_vehicle ~= 0 then
            ...
            if isVehicleELS then
                if GetPedInVehicleSeat(current_vehicle, -1) == ped or GetPedInVehicleSeat(current_vehicle, 0) == ped then
                    if playerLoaded and playerData ~= nil then
                        if (playerData.job.job == "police" or playerData.job.job == "ems" or playerData.job.job == "fire") and playerData.job.duty then
                            canControlELS = true
                        else
                            canControlELS = false
                        end
