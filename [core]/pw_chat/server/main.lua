AddEventHandler('chatMessage', function(source, n, message)
    local char = exports.pw_base:Source(source)
    if char ~= nil then
        local cData = char:Character()
            ...
            if commands[commandName] ~= nil then
                if commands[commandName].job ~= nil then
                    for k, v in pairs(commands[commandName].job) do
                        if char:Job():getJob().job == v.name then
...
AddEventHandler('pw:chat:server:311Alert', function(name, location, message)
    ...
    local players = exports.pw_base:getOnlineCharacters()
    
    for i = 1, #players, 1 do
        local cData = exports.pw_base:Source(players[i].source)
        if cData:Job():getJob().job == 'police' and cData:Job():getDuty() or cData:Job():getJob().job == 'ems' and cData:Job():getDuty() then
            TriggerClientEvent('chat:addMessage', cData:Character().getSource(), {
...
AddEventHandler('pw:chat:server:911Alert', function(name, location, message)
    ...
    for i = 1, #players, 1 do
        local cData = exports.pw_base:Source(players[i].source)
        if cData:Job():getJob().job == 'police' and cData:Job():getDuty() then
            TriggerClientEvent('chat:addMessage', cData:Character().getSource(), {
...
AddEventHandler('pw:chat:server:policeDispatch', function(code, locale, message, coords, gender)
    ...
    for i = 1, #players, 1 do
        ...
        if cData:Job():getJob().job == 'police' and cData:Job():getDuty() then
            ...
            TriggerClientEvent('pw_police:displayCrimeBlip', cData:Character().getSource(), code, coords)
        end
    end
    CancelEvent()
end)
...
AddEventHandler('pw:chat:server:emsDispatch', function(name, location, message)
    ...
    for i = 1, #players, 1 do
        local cData = exports.pw_base:Source(players[i].source)
        if cData:Job():getJob().job == 'ems' and cData:Job():getDuty() then
            TriggerClientEvent('chat:addMessage', cData:Character().getSource(), {
