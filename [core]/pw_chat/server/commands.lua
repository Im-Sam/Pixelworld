...
function AddChatCommand(command, callback, suggestion, arguments, job)
    ...
    RegisterCommand(command, function(source, args, rawCommand)
        local cData = exports.pw_base:Source(source)
        if((#args <= commands[command].arguments and #args == commands[command].arguments) or commands[command].arguments == -1)then
            if commands[command].job ~= nil then
                for k2, val in pairs(commands[command].job) do
                    if tostring(val) == cData:Job():getJob().job and cData:Job():getDuty() then
...
function AddAdminChatCommand(command, callback, suggestion, arguments)
    ...
    RegisterCommand(command, function(source, args, rawCommand)
        ...
        if((#args <= commands[command].arguments and #args == commands[command].arguments) or commands[command].arguments == -1) then
            if mPlayer:User():getPermission() == 'Owner' or mPlayer:User():getPermission() == 'Admin' or mPlayer:User():getPermission() == 'Developer' then
                callback(source, args, rawCommand)
            end
        else
            TriggerEvent('pw:chat:server:Server', source, "Invalid Number Of Arguments")
...
AddChatCommand('ad', function(source, args, rawCommand)
    local mPlayer = exports.pw_base:Source(source)
    local fal = mPlayer:Character():getName()
    local phone = mPlayer:Phone():getActiveNumber()
    local msg = rawCommand:sub(4)
    TriggerEvent('pw:chat:server:Advert', fal, '#'..phone, msg)
end, {
...
AddChatCommand('311', function(source, args, rawCommand)
    local mPlayer = exports.pw_base:Source(source):Character()
    local name = mPlayer.getName()
    fal = name
...
AddAdminChatCommand('noclip', function(source, args, rawCommand)
    if source > 0 then 
            TriggerClientEvent("pw_base:noclip", source)
        end
    end, {
            help = "Enable / Disable NoClipping",
    }, -1)