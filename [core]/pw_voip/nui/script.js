...
function init() {
	...
	websocket.onmessage = (evt) => {
		...
		if (evt.data == 'startedtalking') {
			$.post('http://pw_voip/setPlayerTalking', JSON.stringify({state: 1}));
		}
		if (evt.data == 'stoppedtalking') {
			$.post('http://pw_voip/setPlayerTalking', JSON.stringify({state: 0}));
		}
	};
...
function displayPluginScreen(toggle) {
	if(toggle === true) {
		if(togglingView === false) { 
			togglingView = true
			$.post(`http://pw_voip/stopControlKeys`, JSON.stringify({ toggle: true }));
		}
	} else {
		if(togglingView === true) {
			togglingView = false
			$.post(`http://pw_voip/stopControlKeys`, JSON.stringify({ toggle: false }));
		}
	}
...
function updateTokovoipInfo(msg) {
	...
	document.getElementById('pluginStatus').innerHTML = `<font color="${color}">${screenMessage || msg}</font>`;
}

function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `${voip.plugin_data.TSServer}`;
	document.getElementById('TSChannel').innerHTML = `${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}`;
...
function updateScriptData(key, data) {
	...
	$.post(`http://pw_voip/updatePluginData`, JSON.stringify({
		payload: {
			key,
			data,
		}
