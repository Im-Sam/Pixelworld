...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	local info = "";
	if (self.mode == 1) then
		info = "<i class='fad fa-microphone-stand m-1 fa-2x fa-fw'></i>";
		TriggerEvent('pw_voip:client:updateNUI', "level", 50);
	elseif (self.mode == 2) then
		info = "<i class='fad fa-microphone-stand m-1 fa-2x fa-fw'></i>";
		TriggerEvent('pw_voip:client:updateNUI', "level", 25);
	elseif (self.mode == 3) then
		info = "<i class='fad fa-microphone-stand m-1 fa-2x fa-fw'></i>";
		TriggerEvent('pw_voip:client:updateNUI', "level", 100);
	end

	if (self.plugin_data.radioTalking) then
		if (self.myChannels[self.plugin_data.radioChannel].id > 999) then
			info = "<i class='fad fa-mobile-alt m-1 fa-2x fa-fw'></i>"
		else
			info = "<i class='fad fa-walkie-talkie m-1 fa-2x fa-fw'></i>";
		end
	end
	if (self.talking == 1 or self.plugin_data.radioTalking) then
		info = "<font class='text-warning'>" .. info .. "</font>";
	end
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if (string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call")) then
			radio = "In<br>" .. self.myChannels[self.plugin_data.radioChannel].name;
		else
			radio = "Radio<br>" .. self.myChannels[self.plugin_data.radioChannel].name;
		end
		TriggerEvent('pw_voip:client:updateNUI', "secondary", radio);
	else
		TriggerEvent('pw_voip:client:updateNUI', "secondary", false);
	end
	if (info == self.screenInfo and not forceUpdate) then return end
	self.screenInfo = info;
	self:updatePlugin("updateTokovoipInfo", "" .. info);
	TriggerEvent('pw_voip:client:updateNUI', "primary", info);
end

function TokoVoip.updatePlugin(self, event, payload)
	exports.pw_voip:doSendNuiMessage(event, payload);
end
