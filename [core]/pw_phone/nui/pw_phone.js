...
window.addEventListener("message", function (event) {
    ...
    if(event.data.status == "openRadio") {
        if(currentRadioChannel !== null && parseInt(currentRadioChannel) > 99 && parseInt(currentRadioChannel) < 1000) {
...
function receivingCall(incomming, name, failed, reason) {
    ...
    $('#receivingCall').html('');
    $('#receivingCall').append('<div class="row"><div class="col-12 pt-3 text-center"><i id="callBlinker" class="fad fa-phone-volume fa-9x text-success"></i><div class="alert alert-info mt-4 text-center"><small>Incomming Call</small><br><span class="h5 mb-1">' + name + '</span></div></div></div><div class="row"><div class="col-12 pt-3 text-center"><i class="fad fa-times-circle fa-3x text-danger mr-4" data-act="rejectCall"></i> <i class="ml-4 fad fa-check-circle fa-3x text-success" data-act="acceptCall"></i></div></div>')
...
function loadContestants(contestants) {
    ...
    /* <div class="btn-group mx-auto w-100 mb-1" role="group" aria-label="Contestants">
        <div class="btn-group" role="group">
            <button id="btnGroupDrop1" type="button" class="btn btn-info dropdown-toggle btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-tooltip="tooltip" data-placement="top" title="Set Pole Position">' + curPosition + '</button>'
            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <a class="dropdown-item" href="#">#2</a>
                <a class="dropdown-item" href="#">#3</a>
            </div>
        </div>
        <button type="button" class="btn btn-primary btn-sm text-truncate" data-toggle="tooltip" data-placement="top" title="Racer Name 123123123">Racer Name 123123123</button>
        <button type="button" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" title="Move Up"><i class="fas fa-arrow-alt-up fa-fw"></i></button>
        <button type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Move Down"><i class="fas fa-arrow-alt-down fa-fw"></i></button>
    </div> */
...
$(function() { 
    ...
    $(document).on('click','[data-act=setRadioChannel]' ,function(){
        ...
        if(requestedChannel !== undefined && requestedChannel !== null && requestedChannel !== "" && parseInt(requestedChannel) > 99 && parseInt(requestedChannel) < 1000) {
